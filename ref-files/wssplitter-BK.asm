...
#define MIPS  * 4000000 ;4 clock cycles per instr
...
#define ISBANKED(reg)  ((BANKOFS(reg) >= 0x0C) && (BANKOFS(reg) < 0x70))
...
asmbit += 1
    endw
    ERRIF log2(1024) != d'10', [ERROR] LOG2_ constants are bad: LOG2_1024 = #v(log2(1024)), should be #v(1<<d'10')  ;paranoid self-check
    ERRIF log2(1) | log2(0), [ERROR] LOG2_ constants are bad: LOG2_1 = #v(log2(1)) and LOG_0 = #v(log2(0)), should be 0  ;paranoid self-check
...
#define BDCL  ALLOC_GPR TRUE, ; banked alloc
#define NBDCL  ALLOC_GPR FALSE, ; non-banked alloc
...
#define SIZEOF(name)  name#v(0)size
...
ALLOC_GPR MACRO banked, name, numbytes
    NOEXPAND  ;reduce clutter
;    EXPAND_PUSH TRUE  ;show RAM allocations in LST
    EXPAND ;show RAM allocations in LST
    CBLOCK NEXT_RAM#v(BOOL2INT(banked))  ;continue where we left off last time
	name numbytes
...
RAM_BLOCK += 1  ;need a unique symbol name so assembler doesn't complain; LOCAL won't work inside CBLOCK
    ...
    ENDC
NEXT_RAM#v(BOOL2INT(banked)) = LATEST_RAM#v(RAM_BLOCK)  ;update pointer to next available RAM location
RAM_USED#v(BOOL2INT(banked)) = NEXT_RAM#v(BOOL2INT(banked)) - RAM_START#v(BOOL2INT(banked))
    CONSTANT name#v(0)size = LATEST_RAM#v(RAM_BLOCK) - name;
    ERRIF LATEST_RAM#v(RAM_BLOCK) > MAX_RAM#v(BOOL2INT(banked)), [ERROR] ALLOC_GPR: RAM overflow #v(LATEST_RAM#v(RAM_BLOCK)) > max #v(MAX_RAM#v(BOOL2INT(banked))),
...
#define clrw  my_clrw; override default opcode for WREG tracking
;WREG tracking:
clrw macro
    mov8 WREG, LITERAL(0);
    endm

;1 instr takes 2 cycles:
nop2 macro
    goto $+1
    endm
...
;    messg src, dest;
	...
	if !LIT2VAL(SRC) && (DEST != WREG) && !(DEST & INDF0_special) && !(DEST && INDF1_special)
	    BANKCHK dest;
	    BANKSAFE clrf dest, NOARG; REG2ADDR(DEST); ;special case
	    exitm
	endif
	if WREG_TRACKER != src
...
;special pseudo-reg:
	if src & INDF0_special
		...
		if (SRC != WREG) && (SRC != WREG_TRACKER)
		    BANKCHK src;
;        errorlevel -302 ;kludge: Disable bank switch warning
		    BANKSAFE movf src, W; REG2ADDR(SRC), W
;	errorlevel +302 ;kludge: Enable bank switch warning
		    NOEXPAND  ;reduce clutter
WREG_TRACKER = src
		else
	...
	if dest & INDF1_special
		...
		BANKCHK dest;
;    errorlevel -302 ;kludge: Disable bank switch warning
		BANKSAFE movwf dest, NOARG; REG2ADDR(DEST)
		NOEXPAND  ;reduce clutter
;    errorlevel +302 ;kludge: Enable bank switch warning
	    endif
        endif
...
;    messg src, dest;
    ...
    NOEXPAND  ;reduce clutter
    if BOOL2INT(bitval)
        BANKSAFE bsf dest, bit;
        NOEXPAND  ;reduce clutter
    else
	BANKSAFE bcf dest, bit;
        NOEXPAND  ;reduce clutter
    endif
    if dest == WREG
...
;    messg src, dest;
    BANKCHK reg;
    if BOOL2INT(bitval)
        BANKSAFE btfsc reg, bitnum;
	NOEXPAND  ;reduce clutter
    else
	BANKSAFE btfss reg, bitnum;
        NOEXPAND  ;reduce clutter
    endif
;    LOCAL BEFORE_STMT = $
...
;    LOCAL AFTER_STMT = 0; $ - (BEFORE_STMT + 1)
    ERRIF STMT_INSTR != 1, [ERROR] if-ed stmt !1 opcode: #v(STMT_INSTR),
    endm
...
;pic-as, not mpasm: PSECT   code
    ...
    ORG RESET_VECTOR
    nop; reserve space for ICE debugger?
    goto init ;main
...
#define T0_prescale  log2(FOSC_FREQ / 4 / (1 MHz)); set pre-scalar for 1 usec ticks
;    messg ^^ REINSTATE
...
tmr0_init macro
    ...
    mov8 T0CON1, LITERAL(T0SRC_FOSC4 << T0CS0 | NOBIT(T0ASYNC) | T0_prescale << T0CKPS0); FOSC / 4, sync, 8:1 pre-scalar
...
;    messg [INFO] TMR2 period #v(PR2VAL - 1) = PR2VAL - 1, PWM3 duty #v(DUTY) = DUTY
    messg [INFO], Fosc = #v(FOSC_FREQ) == 4 MIPS? #v(FOSC_FREQ == 4 MIPS), WS bit freq = #v(WSBIT_FREQ), #instr/wsbit = #v(FOSC_FREQ/4 / WSBIT_FREQ)
    messg TMR2 period = #v(PR2VAL - 1) = PR2VAL - 1, TMR2 2x = #v(PR2VAL_2x)
tmr2_init macro
...
;    setbit PWM3CON, PWM3EN, TRUE;
    ...
    mov8 T2PR, LITERAL(PR2VAL - 1); 8 MIPS / 10 = 800 KHz; gives 4 bits res?
...
;not used    mov8 SSP1CLKPPS, LITERAL(16);
    ...
    mov8 SSP1STAT, LITERAL(NOBIT(SMP) | NOBIT(CKE)); SPI master mode, xmit on rising clock edge, sample at middle
    messg CKE?
;#define SPI_TMR2_half  (b'0011' << SSPM0); use Timer2/2
    mov8 SSP1ADD, LITERAL(PR2VAL - 1); SPI baud rate: 800KHz (1.25 usec) with 16MHz osc (4 MIPS, 0.25 usec) = 5
;    mov8, RA0PPS
...
;#define BITBANG
#ifdef BITBANG
    messg [INFO] SPI bit bang+busy wait
nop7: nop;
nop6: nop;
nop5: nop;
nop4: return;
...
;WS "0" = 10000 (1+4), "1" = 11100 (3+2) @4MIPS
    ERRIF !want_wait, [ERROR] bit-bang is synchronous, wait required
    LOCAL BYTE = byte
    if !ISLIT(BYTE)
...
;    BDCL wsnodes:#v(NUMPX/2); 1 nibble per node (color indexed); FSR# handles banking
    BDCL wsnodes,:24+8; 1 byte per node (color indexed): 24px for first rcv node + 8 px for fps; FSR# handles banking
#define END_DETECT  0X40; BIT(5)
;    CONSTANT wseof = ENDOF(wsnodes); line too long
    messg [INFO], wsnodes #v(wsnodes), eof #v(ENDOF(wsnodes)), detect #v(END_DETECT)
...
;    sub8 LITERAL(WSBIT_THRESHOLD), WREG
    addlw -WSBIT_THRESHOLD; Borrow => WS "0" bit received, Carry => WS "1" bit received
;    lslf ch1qlen
;    BANKCHK LO(chqlen);
...
;    xorwf LATA, F;
    ...
    DROP_CONTEXT;
;    call wsnodes_clear;
    mov8 INDF1_postinc, LITERAL(OFF_RGBINX); turn it off again before moving to next node (more efficient than clearing entire node buf)
    ifbit FSR1L, log2(END_DETECT), !(wsnodes & END_DETECT), goto ani_loop; !eof
;    xorlw OFF_RGBINX;
...
eof: ;//sanity checks, perf stats
    WARNIF TRUE, [INFO] bank sel perf: #v(BANKSEL_KEEP) kept, #v(BANKSEL_DROP) dropped; ;perf stats
;    MESSG "TODO: fix eof page check"
    ...
    WARNIF TRUE, [INFO] banked RAM used: #v(RAM_USED#v(TRUE))/#v(RAM_LEN#v(TRUE)), non-banked used: #v(RAM_USED#v(FALSE))/#v(RAM_LEN#v(FALSE))
;    END
;#else //!WANT_HOIST
;#endif ;WANT_HOIST
;    messg "hello 5"
